项目已迁移至gitlab

# 精简CI/CD架构设计方案（初期MVP版）

**适用场景：** 10个微服务 + 3-4台云服务器 + 初期项目  
**设计原则：** 快速上线 → 避免过度设计 → 按需演进  
**目标：** 6个月内快速验证业务，同时保留未来扩展空间

---

## 目录
- [1. 整体架构](#1-整体架构)
- [2. 核心理念](#2-核心理念)
- [3. 环境划分](#3-环境划分)
- [4. 组件选型](#4-组件选型)
- [5. 基础设施](#5-基础设施)
- [6. 流程设计](#6-流程设计)
- [7. 成本与性能](#7-成本与性能)
- [8. 实施路线](#8-实施路线)
- [9. 风险与应对](#9-风险与应对)
- [10. 未来演进](#10-未来演进)

---

## 1. 整体架构

### 1.1 架构全景图

```
┌─────────────────────────────────────────────────────────────┐
│                    代码管理 (GitLab)                          │
│              (SaaS 或 Self-hosted 可选)                       │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│         GitLab CI (Runner on K3s Cluster)                    │
│  • 触发CI流程  • 执行构建  • 推送镜像到Harbor                 │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│              单个K3s集群 (3节点，共24GB内存)                   │
├─────────────────────────────────────────────────────────────┤
│ Namespace: platform    Namespace: dev    Namespace: staging  │
│ ├─ Harbor              ├─ App Services   └─ App Services     │
│ ├─ ArgoCD              ├─ PostgreSQL                          │
│ ├─ Prometheus          ├─ Redis                              │
│ ├─ Grafana             └─ Testing                            │
│ └─ Cert-Manager                                              │
│                       Namespace: production                  │
│                       └─ App Services (独立磁盘/网络)         │
└─────────────────────────────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
│            GitOps 仓库 (ArgoCD 监控)                         │
│  • 应用配置版本控制  • 配置与代码分离  • 快速回滚             │
└─────────────────────────────────────────────────────────────┘
```

### 1.2 部署高层流程

```
1. 开发者 git push / merge PR
        ↓
2. GitLab Webhook 触发 CI 流程
        ↓
3. GitLab Runner 执行：
   ├─ 单元测试
   ├─ 代码检查 (SonarCloud)
   ├─ 构建Docker镜像
   └─ 推送到 Harbor
        ↓
4. CI 流程更新 GitOps 仓库 (更新镜像标签)
        ↓
5. ArgoCD 检测到变更，自动部署到相应环境
        ↓
6. 监控和日志自动收集
```

---

## 2. 核心理念

### 2.1 简单胜于完美

| 维度 | 原架构 | 精简方案 | 原因 |
|------|--------|---------|------|
| **集群数量** | 4个 | 1个 | 单集群可支撑到30个微服务 |
| **管理工具** | Rancher | kubectl | 1个集群不需要Rancher |
| **CI引擎** | BuildKite + Tekton | GitLab CI | 减少维护负担，功能足够 |
| **依赖包仓库** | Nexus | Harbor代理 | Harbor有代理功能 |
| **构建系统** | Bazel | Docker Build | 10个微服务不需要Bazel |
| **监控** | 3层分层 | 单层本地 | 初期Prometheus + Grafana足够 |
| **链路追踪** | Jaeger | 暂不引入 | 等服务>30个再考虑 |

### 2.2 快速迭代

从最小MVP开始，逐步完善：

```
Week 1-2: 基础设施搭建 (K3s + Harbor + ArgoCD)
    ↓
Week 3-4: 迁移第1个微服务，验证流程
    ↓
Week 5-6: 迁移剩余微服务
    ↓
Week 7-8: 优化和文档
    ↓
Month 3+: 根据实际情况迭代（添加新功能、性能优化）
```

---

## 3. 环境划分

### 3.1 三环境设计

```yaml
┌──────────────────┬──────────────┬───────────────┬──────────────┐
│ 环境             │ Namespace    │ 用途          │ 资源规格     │
├──────────────────┼──────────────┼───────────────┼──────────────┤
│ Development      │ dev          │ 所有分支都    │ requests:    │
│                  │              │ 部署到这里    │ 0.2C 256M    │
│                  │              │ (最小化)      │ limits:      │
│                  │              │              │ 0.5C 512M    │
├──────────────────┼──────────────┼───────────────┼──────────────┤
│ Staging          │ staging      │ main分支      │ requests:    │
│                  │              │ 通过的代码    │ 0.5C 512M    │
│                  │              │ (完整配置)    │ limits:      │
│                  │              │              │ 1C 1Gi       │
├──────────────────┼──────────────┼───────────────┼──────────────┤
│ Production       │ production   │ 生产流量      │ requests:    │
│                  │              │ (高可用)      │ 1C 2Gi       │
│                  │              │              │ limits:      │
│                  │              │              │ 2C 4Gi       │
└──────────────────┴──────────────┴───────────────┴──────────────┘
```

### 3.2 分支与环境的对应关系

```
Feature Branch
    ↓ Create PR
    ↓ (CI运行测试)
    ↓
dev 分支 → Deploy to dev namespace → QA测试
    ↓ (通过QA)
    ↓ Merge to main
main 分支 → Deploy to staging namespace → UAT
    ↓ (通过UAT，手动批准)
    ↓ Create Release Tag
release-* → Deploy to production → 用户访问
```

### 3.3 何时清理环境

| 环境 | 保留策略 | 清理策略 |
|------|---------|---------|
| **dev** | 持续保留，实时更新 | 手动删除（磁盘满时） |
| **staging** | 持续保留，仅main分支更新 | 手动清理（保留最近10个版本） |
| **production** | 持续保留 | 版本管理（保留最近20个版本） |

---

## 4. 组件选型

### 4.1 基础设施

```
操作系统: Ubuntu 20.04 LTS (云服务器自带)
    ↓
K3s (Kubernetes 发行版)
├─ 特点: 轻量级 (<1GB内存), 一键安装
├─ 替代品: K8s (但太重，不推荐)
└─ 安装: curl -sfL https://get.k3s.io | sh -
    ↓
Helm (包管理工具)
├─ 用途: 安装所有第三方服务
└─ 安装: curl https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 | bash
```

### 4.2 代码仓库

```
GitLab CE
├─ 功能: 完整的Git + CI/CD + Container Registry
├─ 资源: ~1.5GB内存 (shared_buffers较低时)
├─ 维护: 每月自动升级
└─ 部署: Helm Chart (gitlab/gitlab-ce)
```

### 4.3 容器镜像仓库

```
Harbor
├─ 功能: 
│  ├─ 存储Docker镜像
│  ├─ 代理PyPI/npm/Go Mod等 (无需Nexus)
│  ├─ 漏洞扫描 (Trivy集成)
│  └─ RBAC权限管理
├─ 资源: ~0.5GB内存 + PostgreSQL
└─ 部署: Helm Chart (harbor/harbor)
```

### 4.4 CI/CD引擎

```
GitLab CI Runner
├─ 工作原理:
│  ├─ GitLab发送Webhook
│  ├─ Runner接收任务
│  ├─ 在K8s Pod中执行脚本
│  └─ 推送结果回GitLab
├─ 资源占用: <100MB (idle状态)
├─ 并发能力: 可同时运行10+个Pod
└─ 部署: Helm Chart (gitlab-runner/gitlab-runner)
```

### 4.5 GitOps部署引擎

```
ArgoCD 
├─ 职责:
│  ├─ 持续监控GitOps仓库
│  ├─ 自动同步应用到K8s
│  ├─ 提供Web UI查看部署状态
│  └─ 支持快速回滚
├─ 资源: ~0.3GB内存
├─ 配置: Kustomize (无需Helm)
└─ 部署: Helm Chart (argo/argo-cd)
```

### 4.6 监控与日志

#### 监控方案

**最小化监控（启动阶段推荐）**
```
云厂商监控 + metrics-server
├─ 云厂商监控 (腾讯云/阿里云):
│  ├─ 服务器级别监控 (CPU/内存/磁盘)
│  ├─ 基础告警能力
│  └─ 资源: 0 (云端)
│
└─ metrics-server:
   ├─ 提供 kubectl top 命令
   ├─ K8s基础资源监控
   └─ 资源: ~50MB内存

部署方式:
  kubectl apply -f https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml

优点: 
  ✅ 极低资源占用
  ✅ 零维护成本
  ✅ 满足初期监控需求
缺点:
  ❌ 无可视化界面
  ❌ 无历史数据存储
```

**路线：**
```
Month 1-2: 最小化监控 → 快速启动
Month 3+:  完整   → 稳定运行后升级
```

#### 日志方案
```
kubectl logs (极简，应急够用)
```

### 4.7 完整服务清单

#### 配置方案对比

**最小化配置（启动阶段）：**
| 服务 | 资源占用 | 是否必需 | 部署方式 |
|------|---------|---------|---------|
| K3s Master | 2GB | ✅ | 云服务器1 |
| K3s Worker | 6GB (shared) | ✅ | 云服务器2-3 |
| GitLab CE | 1.5GB | ✅ | K3s Pod |
| Harbor | 1GB | ✅ | K3s Pod |
| GitLab Runner | 0.1GB | ✅ | K3s Pod |
| ArgoCD | 0.3GB | ✅ | K3s Pod |
| metrics-server | 0.05GB | ✅ | K3s Pod |
| Cert-Manager | 0.1GB | ✅ | K3s Pod |
| **总计** | **≈11GB** | — | **适合3台4C8G** |

---

## 5. 基础设施

### 5.1 云服务器配置

```yaml
云服务商: 腾讯云 TKE (推荐) 或 阿里云 ACK 或 AWS EKS

服务器规格:
  Server-1 (K3s Master + Harbor):
    CPU: 2核
    内存: 4GB
    磁盘: 40GB SSD
    带宽: 峰值100
  
  Server-2 & Server-3 (K3s Worker):
    CPU: 2核 (每个)
    内存: 4GB (每个)
    磁盘: 40GB SSD (每个)
    带宽: 5Mbps

```

### 5.2 存储方案

```
Pod存储 (临时):
  └─ K3s默认 (local storage)
    ├─ 用于: 应用Pod的工作文件
    └─ 特点: Pod删除后自动清理
```

### 5.3 网络配置

```
DNS: 云厂商提供
  ├─ 分配一个域名 (gitlab.lausencake.com)
  └─ 配置CNAME指向负载均衡器

负载均衡:
  ├─ K3s内置 (默认用Service NodePort)
  └─ 或用云厂商SLB + Ingress Controller

防火墙规则:
  ├─ 入站: 80/443 (HTTP/HTTPS) 对所有
  ├─ 入站: 22 (SSH) 仅运维网络
  └─ 出站: 全部允许 (拉取依赖包)
```

### 5.4 安全配置

```
证书管理:
  └─ Cert-Manager + Let's Encrypt
    ├─ 自动申请HTTPS证书
    ├─ 自动续期
    └─ 无需手动维护

密钥管理:
  └─ K8s Secrets (初期用这个)
     └─ 存储数据库密码、镜像拉取凭证等


镜像安全:
  ├─ Harbor启用镜像扫描 (Trivy)
  ├─ 只允许拉取已扫描的镜像
  └─ 定期扫描已部署的镜像
```

---

## 6. 流程设计

### 6.1 CI流程详解

```yaml
# .gitlab-ci.yml 示例

stages:
  - test
  - build
  - deploy

variables:
  DOCKER_TLS_CERTDIR: ""
  REGISTRY: harbor.lausencake.com.cn
  DOCKER_HOST: tcp://docker:2375

# ==================== TEST阶段 ====================
test:unit:
  stage: test
  image: python:3.10
  script:
    - pip install -r requirements.txt
    - pytest tests/ --cov=app
  coverage: '/TOTAL.*\s+(\d+%)$/'
  only:
    - merge_requests
    - main

test:lint:
  stage: test
  image: python:3.10
  script:
    - pip install flake8 black
    - black --check app/
    - flake8 app/
  only:
    - merge_requests
    - main

test:security:
  stage: test
  image: returntocorp/semgrep
  script:
    - semgrep --config=p/owasp-top-ten app/
  allow_failure: true
  only:
    - merge_requests
    - main

# ==================== BUILD阶段 ====================
build:image:
  stage: build
  image: docker:latest
  services:
    - docker:dind
  script:
    # 生成镜像标签
    - export IMAGE_TAG="${REGISTRY}/myapp:${CI_COMMIT_SHA:0:8}"
    - export IMAGE_TAG_LATEST="${REGISTRY}/myapp:latest"
    
    # 构建镜像
    - docker build -t ${IMAGE_TAG} -t ${IMAGE_TAG_LATEST} .
    
    # 登录Harbor
    - docker login -u ${HARBOR_USERNAME} -p ${HARBOR_PASSWORD} ${REGISTRY}
    
    # 推送镜像
    - docker push ${IMAGE_TAG}
    - docker push ${IMAGE_TAG_LATEST}
    
    # 可选: 扫描镜像漏洞
    - echo "镜像已推送到Harbor，漏洞扫描由Harbor自动执行"
  only:
    - main
    - tags

# ==================== DEPLOY阶段 ====================
deploy:dev:
  stage: deploy
  image: bitnami/kubectl:latest
  script:
    # 更新dev namespace的部署
    - kubectl set image deployment/myapp 
        myapp=${REGISTRY}/myapp:${CI_COMMIT_SHA:0:8} 
        -n dev
    
    # 等待部署完成
    - kubectl rollout status deployment/myapp -n dev
  environment:
    name: dev
    kubernetes:
      namespace: dev
  only:
    - main
  when: manual

deploy:staging:
  stage: deploy
  image: bitnami/kubectl:latest
  script:
    # 更新staging namespace
    - kubectl set image deployment/myapp 
        myapp=${REGISTRY}/myapp:${CI_COMMIT_SHA:0:8} 
        -n staging
    
    - kubectl rollout status deployment/myapp -n staging
  environment:
    name: staging
    kubernetes:
      namespace: staging
  only:
    - main
  when: manual

deploy:production:
  stage: deploy
  image: bitnami/kubectl:latest
  script:
    # 更新production namespace
    - kubectl set image deployment/myapp 
        myapp=${REGISTRY}/myapp:${CI_COMMIT_SHA:0:8} 
        -n production
    
    - kubectl rollout status deployment/myapp -n production
  environment:
    name: production
    kubernetes:
      namespace: production
  only:
    - tags
  when: manual
```

### 6.2 GitOps流程详解

**Kustomize + ArgoCD**

```
gitops-repo/
├── base/
│   ├── kustomization.yaml
│   ├── deployment.yaml
│   ├── service.yaml
│   └── configmap.yaml
│
├── overlays/
│   ├── dev/
│   │   └── kustomization.yaml  (replicas: 1)
│   │
│   ├── staging/
│   │   └── kustomization.yaml  (replicas: 2)
│   │
│   └── production/
│       └── kustomization.yaml  (replicas: 3)
│
└── argocd/
    ├── dev-app.yaml
    ├── staging-app.yaml
    └── production-app.yaml
```

**base/kustomization.yaml:**
```yaml
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization

namespace: default

resources:
  - deployment.yaml
  - service.yaml
  - configmap.yaml

images:
  - name: myapp
    newTag: latest  # ArgoCD会自动更新
```

**overlays/dev/kustomization.yaml:**
```yaml
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization

bases:
  - ../../base

namespace: dev

replicas:
  - name: myapp
    count: 1

patches:
  - target:
      kind: Deployment
      name: myapp
    patch: |-
      - op: replace
        path: /spec/template/spec/containers/0/resources/requests/memory
        value: "256Mi"
```

**argocd/dev-app.yaml:**
```yaml
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
  name: myapp-dev
  namespace: argocd
spec:
  project: default
  source:
    repoURL: https://github.com/mycompany/gitops-repo.git
    targetRevision: main
    path: overlays/dev
  destination:
    server: https://kubernetes.default.svc
    namespace: dev
  syncPolicy:
    automated:
      prune: true      # 自动删除不在Git中的资源
      selfHeal: true   # 自动修复漂移
    syncOptions:
      - CreateNamespace=true
```

**CI流程更新镜像标签：**
```bash
# 在.gitlab-ci.yml中
- git clone https://github.com/mycompany/gitops-repo.git
- cd gitops-repo
- kustomize edit set image myapp=${REGISTRY}/myapp:${CI_COMMIT_SHA:0:8}
- git add .
- git commit -m "Update image tag to ${CI_COMMIT_SHA:0:8}"
- git push origin main
```

### 6.3 回滚流程

**快速回滚（ArgoCD）：**
```bash
# 查看部署历史
argocd app history myapp-production

# 回滚到上一个版本
argocd app rollback myapp-production

# 回滚到指定版本
argocd app rollback myapp-production 3
```

**安全的回滚（Git）：**
```bash
# GitOps仓库中反向提交
git revert <commit-hash>
git push origin main

# ArgoCD自动检测变更并同步
```

---

## 7. 成本与性能

### 7.1 资源利用率预测

```
初期(2-3个月):
├─ CPU利用率: 20-30% (充足)
├─ 内存利用率: 40-50% (有余量)
└─ 磁盘利用率: 30-40% (镜像库会逐步增长)

中期(3-6个月):
├─ CPU利用率: 40-60% (仍有余量)
├─ 内存利用率: 60-70% (需关注)
└─ 磁盘利用率: 60-70% (需要清理旧镜像)

扩展触发点:
├─ CPU持续>80% → 增加Worker节点
├─ 内存持续>80% → 增加内存或优化Pod配置
└─ 磁盘>90% → 清理旧镜像或升级存储
```

### 7.2 性能指标目标

| 指标 | 目标 | 说明 |
|------|------|------|
| **CI流程耗时** | <10分钟 | 从push到镜像推送完成 |
| **部署耗时** | <5分钟 | 从镜像推送到应用ready |
| **应用启动时间** | <30秒 | 容器启动到ready probe通过 |
| **镜像扫描** | <2分钟 | Harbor漏洞扫描完成 |
| **API响应时间** | <500ms | P95延迟 |
| **系统可用性** | >95% | 允许每月有几小时故障 |

---

## 8. 实施路线

### 8.1 阶段1：基础设施搭建（第1-2周）

**第1周：采购 & 初始化**

```bash
# Step 1: 采购服务器
- 云厂商购买3台4C8G服务器
- 采购域名1个 (例如gitlab.mycompany.com)
- 采购对象存储容量 (100GB起)

# Step 2: 初始化服务器
- 安装Ubuntu 20.04 LTS
- 配置基础安全 (防火墙、密钥对)
- 安装Docker (K3s依赖)

# Step 3: 安装K3s
Server-1 (Master):
  curl -sfL https://get.k3s.io | sh -
  export KUBECONFIG=/etc/rancher/k3s/k3s.yaml

Server-2 & Server-3 (Workers):
  curl -sfL https://get.k3s.io | \
    K3S_URL=https://<server-1-ip>:6443 \
    K3S_TOKEN=<token-from-server-1> sh -

# Step 4: 验证集群
kubectl get nodes
# 应该看到3个nodes处于Ready状态
```

**第2周：平台服务部署**

```bash
# Step 1: 安装Helm
curl https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 | bash

# Step 2: 添加Helm仓库
helm repo add gitlab https://charts.gitlab.io
helm repo add harbor https://helm.goharbor.io
helm repo add argo https://argoproj.github.io/argo-helm
helm repo add prometheus-community https://prometheus-community.github.io/helm-charts
helm repo update

# Step 3: 创建命名空间
kubectl create namespace gitlab
kubectl create namespace harbor
kubectl create namespace argocd
kubectl create namespace monitoring

# Step 4: 部署Harbor
helm install harbor harbor/harbor \
  -n harbor \
  -f harbor-values.yaml

# Step 5: 部署GitLab (可选，也可用外部GitLab)
# 如使用外部GitLab，跳过此步

# Step 6: 部署ArgoCD
helm install argo-cd argo/argo-cd \
  -n argocd \
  --create-namespace

# Step 7: 部署Prometheus + Grafana
helm install prometheus prometheus-community/kube-prometheus-stack \
  -n monitoring

# Step 8: 创建应用命名空间
kubectl create namespace dev
kubectl create namespace staging
kubectl create namespace production
```

### 8.2 阶段2：验证与优化（第3-4周）

```bash
# Step 1: 迁移第一个微服务 (选最简单的一个)
- 编写Dockerfile
- 编写.gitlab-ci.yml
- 提交第一个commit
- 观察CI流程执行情况
- 手动部署到dev环境
- 验证应用是否正常运行

# Step 2: 优化和调试
- 如果镜像体积过大，优化Dockerfile
- 如果CI耗时过长，分析瓶颈
- 如果应用无法启动，检查Pod日志

# Step 3: 编写操作文档
- K8s基础操作指南
- CI/CD流程说明
- 故障排查指南
- 备份恢复流程

# Step 4: 团队培训
- 讲解整体架构
- 演示CI/CD流程
- 演示回滚操作
- 答疑和讨论
```

### 8.3 阶段3：全量迁移（第5-8周）

```
Week 5: 迁移2-3个核心微服务
Week 6: 迁移4-6个微服务
Week 7: 迁移7-10个微服务
Week 8: 完成所有微服务，测试所有流程

关键检查点:
├─ 所有微服务都能通过CI
├─ 所有环境都能自动部署
├─ 监控和告警正常工作
└─ 团队能够独立操作
```

### 8.4 第一月的优先级

```
P0 (必须):
├─ K8s集群搭建
├─ Harbor部署
├─ 第一个微服务迁移
└─ CI流程验证

P1 (重要):
├─ 所有微服务迁移
├─ ArgoCD配置
├─ 基础监控

P2 (可选):
├─ GitLab高可用
├─ 详细的日志系统
└─ 性能优化
```

---

## 9. 风险与应对

### 9.1 常见风险

| 风险 | 概率 | 影响 | 应对方案 |
|------|------|------|---------|
| **K3s集群宕机** | 中 | 高 | 3节点多副本配置，定期备份etcd |
| **Harbor存储满** | 中 | 中 | 定期清理旧镜像，设置GC策略 |
| **GitLab CE升级失败** | 低 | 高 | 升级前备份，测试环境先升级 |
| **密钥泄露** | 低 | 严重 | 使用Sealed Secrets，定期轮转 |
| **数据库连接失败** | 低 | 中 | 使用RDS托管DB，启用多AZ |
| **磁盘空间不足** | 中 | 中 | 定期监控，设置告警 |
| **CI超时** | 低 | 低 | 优化构建步骤，使用缓存 |
| **网络分区** | 低 | 中 | 使用云厂商高可用网络 |

### 9.2 备份策略

```yaml
备份对象:
├─ K8s etcd (集群配置): 每天备份到S3
├─ Harbor数据库: 每天备份到S3
├─ GitLab数据库: 每天备份到S3
└─ 镜像数据: 使用Harbor复制功能镜像到S3

备份频率:
├─ 日备份: 生产关键数据
├─ 周备份: 长期保留
└─ 月备份: 归档

恢复测试:
├─ 每月演练一次恢复流程
├─ 确保RTO < 1小时
└─ 记录恢复步骤
```

### 9.3 故障排查清单

**Pod无法启动：**
```bash
kubectl describe pod <pod-name> -n <namespace>
kubectl logs <pod-name> -n <namespace>
```

**镜像推送失败：**
```bash
docker login <harbor-url>
# 检查Harbor磁盘空间
kubectl exec -it harbor-core-xxx -n harbor -- df -h
```

**GitOps同步失败：**
```bash
argocd app get <app-name>
argocd app sync <app-name>
```

**集群网络不通：**
```bash
kubectl get svc -n production
# 检查Service ClusterIP是否可达
```

---

## 10. 未来演进

### 10.1 3-6个月的可能升级

当满足以下条件时考虑升级：

```
升级条件:
├─ 微服务数量 > 20
├─ 同时进行的CI任务 > 5个
├─ 内存利用率持续 > 75%
└─ 团队人数 > 3人

可选升级:
├─ 添加第二个Worker节点专门用于CI
├─ 引入镜像缓存加速构建 (Harbor proxy cache)
├─ 部署Sealed Secrets加强密钥管理
└─ 配置Harbor + 云存储后端
```

### 10.2 6-12个月的可能升级

```
升级条件:
├─ 微服务数量 > 30
├─ 部署频率 > 50次/天
├─ 需要跨地域部署
└─ 出现合规/审计需求

可能的升级:
├─ 分离CI和应用集群
├─ 引入Service Mesh (初级: Cilium CNI)
├─ 配置多集群管理 (Rancher或Karmada)
├─ 部署Vault进行密钥中心化管理
└─ 实施更细粒度的RBAC
```

### 10.3 演进路线图

```
Month 0-2: MVP完成
  └─ 单集群 + GitLab CI + ArgoCD

Month 2-4: 稳定运行
  └─ 优化性能，完善文档

Month 4-6: 初步扩展
  ├─ 可能添加第二个Worker
  ├─ 引入镜像缓存
  └─ 性能优化

Month 6-12: 多集群准备
  ├─ 评估是否需要多集群
  ├─ 调研Rancher或Karmada
  └─ 规划灾备策略

Month 12+: 规模化
  ├─ Staging环境独立到新集群
  ├─ Production环境多AZ部署
  ├─ 全面实施GitOps + Terraform IaC
  └─ 专业SRE团队接管运维
```

---

## 11. 快速参考

### 11.1 常用命令速查表

```bash
# ========== 集群管理 ==========
kubectl get nodes                          # 查看集群节点
kubectl describe node <node-name>          # 查看节点详情
kubectl top nodes                          # 查看节点资源使用情况

# ========== Pod管理 ==========
kubectl get pods -n <namespace>            # 列出命名空间内的Pod
kubectl describe pod <pod-name>            # 查看Pod详情
kubectl logs <pod-name>                    # 查看Pod日志
kubectl exec -it <pod-name> -- bash        # 进入Pod容器
kubectl delete pod <pod-name> -n <ns>      # 删除Pod

# ========== 部署管理 ==========
kubectl apply -f deployment.yaml           # 应用配置
kubectl rollout status deployment/<name>   # 查看部署状态
kubectl rollout undo deployment/<name>     # 回滚部署
kubectl set image deployment/<name> <image> # 更新镜像

# ========== 故障排查 ==========
kubectl get events -n <namespace>          # 查看事件
kubectl describe deployment <name>         # 查看部署详情
kubectl get pvc -n <namespace>             # 查看持久化卷

# ========== ArgoCD命令 ==========
argocd app list                            # 列出应用
argocd app get <app-name>                  # 查看应用
argocd app sync <app-name>                 # 同步应用
argocd app rollback <app-name>             # 回滚应用
```

### 11.2 文件模板速查表

**Dockerfile:**
```dockerfile
FROM python:3.10-slim
WORKDIR /app
COPY requirements.txt .
RUN pip install -r requirements.txt
COPY . .
CMD ["python", "-m", "uvicorn", "app:app", "--host", "0.0.0.0"]
```

**deployment.yaml:**
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: myapp
spec:
  replicas: 2
  selector:
    matchLabels:
      app: myapp
  template:
    metadata:
      labels:
        app: myapp
    spec:
      containers:
      - name: myapp
        image: harbor.mycompany.com/myapp:latest
        ports:
        - containerPort: 8000
        resources:
          requests:
            cpu: 100m
            memory: 256Mi
          limits:
            cpu: 500m
            memory: 512Mi
```

**service.yaml:**
```yaml
apiVersion: v1
kind: Service
metadata:
  name: myapp
spec:
  selector:
    app: myapp
  type: ClusterIP
  ports:
  - port: 80
    targetPort: 8000
```


## 12. 工作计划总览

### **阶段1：基础设施搭建（第1-2周）**
- **采购资源**：3台4C8G云服务器 + 域名 + 对象存储
- **搭建K3s集群**：1个Master + 2个Worker节点
- **部署平台服务**：Harbor、ArgoCD、Prometheus/Grafana、Cert-Manager
- **配置安全**：DNS、负载均衡、HTTPS证书、防火墙规则

### **阶段2：验证与优化（第3-4周）**
- **首个服务迁移**：选择最简单的微服务验证CI/CD流程
- **流程优化**：优化Dockerfile、CI耗时、资源配置
- **文档编写**：操作指南、故障排查手册、备份恢复流程
- **团队培训**：架构讲解、实操演示、答疑讨论

### **阶段3：全量迁移（第5-8周）**
- **Week 5**：迁移2-3个核心微服务
- **Week 6**：迁移4-6个微服务
- **Week 7**：迁移7-10个微服务
- **Week 8**：完成所有服务，全流程测试，建立监控告警

## 🎯 关键检查点

| 阶段 | 验收标准 |
|------|---------|
| **第2周** | ✅ K3s集群3节点Ready<br>✅ Harbor可访问并推送镜像<br>✅ ArgoCD部署成功 |
| **第4周** | ✅ 首个微服务完整CI/CD流程通过<br>✅ 三环境(dev/staging/prod)可用<br>✅ 团队能独立操作 |
| **第8周** | ✅ 10个微服务全部迁移<br>✅ 监控告警正常工作<br>✅ 备份策略验证通过 |

---

## 总结

| 维度 | 精简方案 |
|------|---------|
| **集群数量** | 1个K3s集群 |
| **服务器** | 3台4C8G (¥2000-3000/月) |
| **核心服务** | GitLab CI + Harbor + ArgoCD |
| **监控** | Prometheus + Grafana |
| **团队规模** | 1-2人 |
| **支持微服务数** | 10-30个 |
| **部署频率** | 3-5次/天 |
| **回滚时间** | <5分钟 |
| **系统可用性** | >95% |

这个方案相比原设计**简化了80%**的复杂度，但仍保留了完整的CI/CD能力和未来扩展空间。

关键成功因素：
1. ✅ 团队理解GitOps理念
2. ✅ 建立清晰的分支策略
3. ✅ 定期备份和测试恢复
4. ✅ 完善的故障排查文档
5. ✅ 循序渐进的学习曲线
